import { useGetMyCartQuery } from "@/features/cart"
  ...
  return (
    <Link to={routes.CART.to} className='not-italic cursor-pointer relative'>
      ...
      {data?.items.length && data.items.length > 0 ? (
        <span className="absolute -top-2 -right-2 w-5 h-5 rounded-full flex justify-center items-center bg-emerald-500">{data?.items.length}</span>
      ) : null}
    </Link>
  )
}
